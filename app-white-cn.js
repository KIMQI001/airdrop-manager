...
function getStatusClass(status) {
    ...
    return classes[status] || classes['Researching'];
}
...
function renderProjects() {
    ...
    if (state.projects.length === 0) {
        container.innerHTML = '<p class="text-slate-400 text-sm py-4 text-center">暂无项目</p>';
        return;
    }
    container.innerHTML = state.projects.map(p => `
        <div class="flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 transition border border-transparent hover:border-slate-200">
            <div class="flex items-center gap-4">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-semibold text-sm">${p.name.charAt(0).toUpperCase()}</div>
                <div>
            ...
            <div class="flex items-center gap-1">
                <span class="px-2.5 py-1 rounded-lg text-xs font-medium ${getStatusClass(p.status)}">${getStatusText(p.status)}</span>
...
function renderActivity() {
    ...
    if (state.activities.length === 0) {
        container.innerHTML = '<p class="text-slate-400 text-sm py-4 text-center">暂无活动</p>';
        return;
    }
...
function showAddProjectModal() {
    showModal('添加项目', `
        <form onsubmit="saveProject(event)">
            <div class="mb-4">
                <label class="block text-sm text-slate-600 mb-2">项目名称</label>
                <input type="text" id="proj-name" placeholder="例如：Uniswap、LayerZero" required class="input">
            </div>
            <div class="mb-4">
                <label class="block text-sm text-slate-600 mb-2">状态</label>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    <label class="block text-sm text-slate-600 mb-2">总任务数</label>
                    <input type="number" id="proj-total" value="0" class="input">
                </div>
                <div>
                    <label class="block text-sm text-slate-600 mb-2">已完成</label>
...
function showSettingsModal() {
    showModal('设置', `
        <form onsubmit="saveSettings(event)">
            <div class="mb-4">
                <label class="block text-sm text-slate-600 mb-2">Binance API 密钥（仅查看）</label>
            ...
            <div class="flex gap-3">
                <button type="button" onclick="hideModal()" class="btn btn-secondary flex-1">取消</button>
                <button type="button" onclick="fetchBinanceBalance()" class="btn flex-1" style="background: #059669; color: white;">获取余额</button>
                <button type="submit" class="btn btn-primary flex-1">保存</button>
            </div>
        </form>
    `);
...
function showAddActivityModal() {
    ...
    showModal('添加活动', `
        <form onsubmit="saveActivity(event)">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    <label class="block text-sm text-slate-600 mb-2">数量</label>
                    <input type="number" id="act-amount" step="0.0001" required class="input" placeholder="0.00">
                </div>
                <div>
                    <label class="block text-sm text-slate-600 mb-2">代币</label>
                    <input type="text" id="act-token" required class="input" placeholder="ETH、BNB...">
                </div>
            </div>
            <div class="mb-4">
                <label class="block text-sm text-slate-600 mb-2">日期</label>
